...
def renew_request(new_request):
    ...
    try:
        # Отправляем запрос на сервер первого бота
        response = requests.post(f"{SOURCE_SERVER_URL}/api/new_request-pdf/",
                                 data=data)
        logger.info(f"{response} received")
...
def send_count(user, count, count_type, limit):
    # Отправляет информацию про загрузки и валидацию пользоватей, которые есть в SciSourceBot(состоят в канале)
    data = {
        'count': count,
    ...
    try:
        # Отправляем запрос на сервер первого бота
        response = requests.post(f"{SOURCE_SERVER_URL}/api/user_counters/",
                                 data=data)
        logger.info(f"{response} received")
        if response.status_code == 200:
            logger.info(
...
def send_pdf(pdfupload):
    ...
    with open(pdfupload.path, 'rb') as f:
        ...
        response = requests.post(f"{SOURCE_SERVER_URL}/api/upload-pdf/",
                                 data=data,
                                 files=files)
        if response.status_code == 200:
