...
from bot.services import check_pdf_file
...
def get_doi_from_filename(name: str):
    """
    Извлекает DOI (/ в названии отсутствует, заменен на ' ') из названия файла.
    Преобразет в правильный формат (c /).
    """
...
async def pdf_file_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Обрабатывает входящее сообщение с PDF-файлом от пользователя.
    Проверяет, что полученный файл — PDF.
    Извлекает DOI из имени файла.
    Если DOI не найден — удаляет сообщение.
    Если DOI найден — вызывает функцию."""
    ...
    # Если имя файла не соответствует формату,
    # то удаляем сообщение с этим файлом
    if not doi:
